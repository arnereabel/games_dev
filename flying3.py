...
class Clouds(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.surface = pygame.image.load('assets/cloud.png')

        self.rect = self.surface.get_rect(
            center = (
                random.randint(screen_width + 50, screen_width + 150),
...
pygame.mixer.init()                                                 # inits the pygame mixer to add sound
...
pygame.mixer.music.load('assets/Flight_Music.wav')                  #loading the music
pygame.mixer.music.play(loops= -1)                                  # accessing our music file in the mixer and play it

collision_sound = pygame.mixer.Sound('assets/collision.wav')        # load an asset
explosion = pygame.image.load('assets/explosion.png')


keeprunning = True                                                  # set a variable as boolean  to keep the main loop alive

score = 0                                                           # sets score variable to zero
...
while keeprunning:                                                  # while keeprunning == True
    ...
    enemies.update()                                                 # handles all the enemies updates all at once
    clouds.update()
    # fill the screen
    ...
    if pygame.sprite.spritecollideany(player, enemies):            # check for the enemy being collided with the player,,  with .sprite you acces al the sprite properties in pygame
        if player.alive:
            collision_sound.play()                                  # play
    ...
    if player.alive():                                               #update player score
        score += 1
    font = pygame.font.SysFont(None, 25)                            #draw the score
